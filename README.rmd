...
**Experiment
The authors initially have empty cells in their matrix. Some communities are saturated. I wanted to know how the population dynamics would change if the starting community were fully saturated with S, C or R cells. I hypothesize that the S cells would die out faster in this scenario because there are no empty cells to be randomly changed to occupied by other cell types, so the overall displacement would play out faster. Results below.
...
#######################################################
##for loop to replicate local sampling
abundance<-1
for(i in 1:1000){
  replicate(2500, local.comp(initial.matrix))
  #C,S,R
  column<-rbind(1, sum(initial.matrix=="1"), sum(initial.matrix=="2"), sum(initial.matrix=="3"))
  abundance<-cbind(abundance,column)
}
#log transform the data
abundance.1<-abundance[,-1]
abundance.1[1,1:1000]<-1:1000
abundance.1[2,]<-sapply(abundance.1[2,],log10)
abundance.1[3,]<-sapply(abundance.1[3,],log10)
abundance.1[4,]<-sapply(abundance.1[4,],log10)
#plot the abundances
plot(abundance.1[1,],abundance.1[2,], type="l", col="green",ylim=(c(0,5)),ylab="Log(abundance)",xlab="Time",main="Local neighbourhood",)
lines(abundance.1[1,],abundance.1[3,], type="l",col="red")
lines(abundance.1[1,],abundance.1[4,],type="l",col="blue")
############################################
####for global sampling
##for loop to replicate local sampling
abundance.global<-1
for(i in 1:500){
  replicate(2500, global.comp(initial.matrix))
  #C,S,R
column<-rbind(1, sum(initial.matrix=="1"), sum(initial.matrix=="2"), sum(initial.matrix=="3"))
  abundance.global<-cbind(abundance.global, column)
}

#log transform the data for global
abundance.global.1<-abundance.global[,-1]
abundance.1.global[1,1:1000]<-1:1000
abundance.1.global[2,]<-sapply(abundance.1.global[2,],log10)
abundance.1.global[3,]<-sapply(abundance.1.global[3,],log10)
abundance.1.global[4,]<-sapply(abundance.1.global[4,],log10)
#plot the abundances for global
plot(abundance.1.global[1,],abundance.1.global[2,], type="l", col="green",ylim=(c(0,5)),ylab="Log(abundance)",xlab="Time",main="Global neighbourhood",)
lines(abundance.1.global[1,],abundance.1.global[3,], type="l",col="red")
lines(abundance.1.global[1,],abundance.1.global[4,],type="l",col="blue")

#############
#loop for experimental
abundance.expt<-1
for(i in 1:1000){
  replicate(2500, experimental.comp(initial.matrix))
  #C,S,R
  column<-rbind(1, sum(initial.matrix=="1"), sum(initial.matrix=="2"), sum(initial.matrix=="3"))
  abundance.expt<-cbind(abundance.expt,column)
}

#log transform the data for experiment
abundance.expt<-abundance[,-1]
abundance.expt[1,1:1000]<-1:1000
abundance.expt[2,]<-sapply(abundance.expt[2,],log10)
abundance.expt[3,]<-sapply(abundance.expt[3,],log10)
abundance.expt[4,]<-sapply(abundance.expt[4,],log10)
#plot the abundances for experiment
plot(abundance.expt[1,],abundance.expt[2,], type="l", col="green",ylim=(c(0,5)),ylab="Log(abundance)",xlab="Time",main="no Empty cells Local neighbourhood",)
lines(abundance.expt[1,],abundance.expt[3,], type="l",col="red")
lines(abundance.expt[1,],abundance.expt[4,],type="l",col="blue")

#extra code to transpose matrix etc. that ended up not using because needed forloop
#####################################################################
### replicate the sampling and changing of cells for local competiton
write.table(abundences(initial.matrix),file="local.txt")
#format data
...
plot(log10(trans.local.results$R)~trans.local.results$counter, type="l", col="green", ylim=(c(0,5), ylab="Log(Abundance)", xlab="Time", main="Local neighbourhood"))
...
#function to get abundences throughout competition rounds
abundences<-function(initial.matrix){
