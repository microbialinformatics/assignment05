...
if( l.lower.cor.col>50)  l.lower.cor.col<-1
  ...
  #change the type based on what is in neighborhood
    ...
    if(initial.matrix[r,c]==4) run.empty<-T
    if(initial.matrix[r,c]==(c(1,2,3))) run.full<-T  
      emptycells<-function(run.empty<-F)    
...
#what to do if there are ties: re-run?
    if(fract.ones==fract.twos) rerun<-T
      #rerun function....
