...
if
Cs<-sample((c(1,2,3)),1,prob=(c((1/3),(.25+(.75*fract.ones)),(10/32))))
if(Cs==1) newcell<-1
if(Cs==2) newcell<-2
if(Cs==3) newcell<-3
#change the cell based on Cs  
  ...
  #probability of it becoming 1,2,3,0 is based on probability 
    if(initial.matrix[r,c]==4) change.type<-sample(1:8,1, replace=F)
    if(change.type==1) newcell==A
    if(change.type==2) newcell==B
    if(change.type==3) newcell==C
    if(change.type==4) newcell==D
    if(change.type==5) newcell==E
    if(change.type==6) newcell==G
    if(change.type==7) newcell==H
    if(change.type==8) newcell==I
...
initial.matrix[r,c]<-newcell
return initial.matrix #need to make this global and store matrix
}

###loop to replicate the sampling and changing of cells
replicate.comp<-function(){
  local.results<<-rep(local.comp(), 50)
  return(write.table(local.results, file="local.txt")
}
###replicate sampling for global
replicate.comp.global<-function(){
  global.results<<-rep(global.comp(),50)
