...
runSimulations <- function(matrix, nsim=3000){
  ...
  if (nrow!=1 & nrow != 250 & ncol != 1 & ncol 1= 250){ # IF IN MIDDLE
  ...
  else if (nrow == 1 & ncol != 1 & ncol != 250){ #only for those in row 1 and NO CORNERS!
    ...
    cell7 <- matrix[nrow+1,ncol]
    cell8 <- matrix[nrow+1,ncol+1]  }
  else if (nrow == 1 & ncol == 250){ #only for those in row 1 and column 250!  TOP RIGHT CORNER
    cell1 <- matrix[nrow(matrix),ncol-1]
    cell2 <- matrix[nrow(matrix),ncol]
  ...
  else if (nrow == 250 & ncol != 1 & ncol != 250){ # NO CORNERS LAST ROW
    cell1 <- matrix[nrow-1,ncol-1]
    cell2 <- matrix[nrow-1,ncol]
    cell3 <- matrix[nrow-1,ncol+1]
    cell4 <- matrix[nrow,ncol-1]
  ...
  else if (nrow == 250 & ncol == 1){ # BOTTOM LEFT CORNER
  ...
  else if (nrow == 250 & ncol == 250){ # BOTTOM RIGHT CORNER!
    cell1 <- matrix[nrow-1,ncol-1]
    cell2 <- matrix[nrow-1,ncol]
  ...
  else if (ncol == 1 & nrow != 1 & nrow != 250){ #middle column 1
    ...
    cell7 <- matrix[nrow+1,ncol]
    cell8 <- matrix[nrow+1,ncol+1]  }    
  else if (ncol == 250 & nrow != 1 & nrow != 250){ #middle last column!
    cell1 <- matrix[nrow-1,ncol-1]
    cell2 <- matrix[nrow-1,ncol]
